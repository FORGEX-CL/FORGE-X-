...
export async function validateFairLaunchMint(connection: Connection, mintAddress: string): Promise<ValidatedLaunchMint> {
  ...
  if (!("parsed" in accountData) || accountData.parsed.type !== "mint") {
    throw new Error("Address is not an SPL mint account");
  }

  const data = accountData.parsed.info as ParsedMintInfo;
  ...
  if (supply !== FORGE_X_SUPPLY * 10n ** BigInt(FORGE_X_DECIMALS)) throw new Error("Fair Launch mint must contain exactly 1,000,000,000 tokens");
