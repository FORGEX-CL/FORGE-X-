...
export type FrontendChainState = { status: "idle" | "preparing" | "awaiting_signature" | "confirming" | "confirmed" | "failed"; signature?: string; error?: string };
const POLL_MS = 500; const TIMEOUT_MS = 90_000;
function sleep(ms: number) { return new Promise((resolve) => setTimeout(resolve, ms)); }
export function chainState(status: FrontendChainState["status"], extra: Omit<FrontendChainState, "status"> = {}): FrontendChainState { return { status, ...extra }; }
async function waitForSignature(connection: Connection, signature: string, lastValidBlockHeight: number): Promise<void> {
  ...
  while (Date.now() - startedAt < TIMEOUT_MS) {
    ...
    if (await connection.getBlockHeight("confirmed") > lastValidBlockHeight) throw new Error("Solana transaction expired before confirmation");
    await sleep(POLL_MS);
  }
...
export async function signAndConfirmFrontendTransaction(
  ...
  try {
    ...
    if (simulation.value.err) return chainState("failed", { error: JSON.stringify(simulation.value.err) });
    ...
    const result = chainState("confirmed", { signature }); onState(result); return result;
  } catch (error) {
    const result = chainState("failed", { error: error instanceof Error ? error.message : "Transaction failed" }); onState(result); return result;
  }
}
