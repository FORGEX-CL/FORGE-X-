...
function address(value: MintLike | undefined) {
  return typeof value === "string" ? value : value?.address;
}

function symbol(value: MintLike | undefined) {
  return typeof value === "string" ? null : value?.symbol ?? null;
}

function decimals(value: MintLike | undefined) {
  return typeof value === "string" ? null : value?.decimals ?? null;
}
...
function normalizePool(pool: PoolLike, expectedProgramId: string) {
  return {
    ...
    mintA: address(pool.mintA) ?? null,
    mintB: address(pool.mintB) ?? null,
    symbolA: symbol(pool.mintA),
    symbolB: symbol(pool.mintB),
    decimalsA: decimals(pool.mintA),
    decimalsB: decimals(pool.mintB),
...
async function verifyPoolAccount(
  connection: Connection,
  pool: PoolLike,
  expectedProgram: PublicKey,
  requestedMint: PublicKey | null,
) {
  const poolAddress = pool.id ? new PublicKey(pool.id) : null;
  if (!poolAddress) return false;
  const account = await connection.getAccountInfo(poolAddress, "confirmed");
  if (!account || !account.owner.equals(expectedProgram)) return false;

  const mintA = address(pool.mintA);
  const mintB = address(pool.mintB);
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (poolId) {
      ...
      const info = isPoolLike(rpcPool.poolInfo) ? rpcPool.poolInfo : null;
      if (!info || info.programId !== expectedProgram.toBase58()) {
        throw new Error("Pool is not an expected Raydium CPMM pool on this network");
      }
      if (!(await verifyPoolAccount(connection, info, expectedProgram, mint))) {
        throw new Error("Pool account failed on-chain verification");
      }
    ...
    const candidates = Array.isArray(rawPools)
      ? rawPools
          .filter(isPoolLike)
          .filter((pool) => pool.programId === expectedProgram.toBase58())
          .slice(0, MAX_RESULTS)
      : [];

    const verifiedPools: PoolLike[] = [];
    for (const pool of candidates) {
      if (await verifyPoolAccount(connection, pool, expectedProgram, mint)) verifiedPools.push(pool);
    }

    return NextResponse.json({
      network,
      verified: true,
      source: "raydium-api+rpc",
      pools: verifiedPools.map((pool) => normalizePool(pool, expectedProgram.toBase58())),
    });
  } catch (error) {
    return NextResponse.json(
