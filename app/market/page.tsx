  ...
  return (
    <Shell>
      <main className="mx-auto max-w-7xl px-5 py-12 lg:px-8">
        <SectionTitle
          eyebrow="Market"
          title="Discover Solana markets."
          text="Search live market pairs and inspect price, 24h movement, volume and liquidity before you trade."
        />
        ...
        <Card>
          <div className="hidden grid-cols-7 gap-4 border-b border-white/10 px-2 pb-4 text-xs uppercase tracking-wider text-white/35 md:grid">
            <span>Pair</span>
            <span>Price</span>
            <span>24h</span>
            <span>Volume</span>
          ...
          {pairs.length === 0 && !loading ? (
            <div className="py-16 text-center text-sm text-white/40">
              Search for a token to load live Solana markets.
            </div>
          ) : (
            pairs.map((pair) => {
              ...
              const forgeLive = pair.forgeStatus === "LIVE";
              const migrated =
              ...
              return (
                <div
                  key={`${pair.pairAddress}-${pair.dexId}`}
                  className="grid gap-3 border-b border-white/5 px-2 py-5 text-sm last:border-0 md:grid-cols-7 md:items-center md:gap-4"
                >
                  <div>
                    <div className="font-semibold">
                      {pair.baseToken?.symbol ?? "Unknown"}/{pair.quoteToken?.symbol ?? "—"}
                    </div>
                    ...
                    <div
                      className={`mt-1 text-[10px] font-bold uppercase tracking-wider ${
                        blocked
                          ? "text-red-300"
                          : risk?.impersonation || risk?.level === "HIGH"
                            ? "text-amber-300"
                            : "text-emerald-300"
                      }`}
                    >
                  ...
                  <span>
                    {pair.priceUsd ? `$${Number(pair.priceUsd).toPrecision(6)}` : "—"}
                  </span>
