...
export function AdvancedLaunchPanel() {
  ...
  async function executeTokenCreation() {
    ...
    setError(""); setTokenSignature(""); setMint(""); setStatus("preparing");
    try {
      ...
      setTokenSignature(txid); await waitForConfirmation(connection, txid);
      ...
      setTokenVerified(true); setStatus("verified");
  ...
  const actionIsPool = tokenVerified && !poolVerified;
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[.025] p-6">
      ...
      <div className="grid grid-cols-4 gap-2">{STEPS.map((label, index) => <button key={label} type="button" onClick={() => index <= step && !tokenVerified && setStep(index)} disabled={tokenVerified} className={`rounded-lg border px-2 py-2 text-xs font-bold disabled:cursor-default ${index === step ? "border-[#f5c542]/50 bg-[#f5c542]/10 text-[#f5c542]" : "border-white/10 text-white/40"}`}><span className="mr-1">0{index + 1}</span>{label}</button>)}</div>
      {step === 0 && <div className="mt-5 grid gap-3 sm:grid-cols-2"><input className="forge-input" placeholder="Token name" value={name} onChange={(e) => setName(e.target.value)} maxLength={32} disabled={tokenVerified}/><input className="forge-input" placeholder="SYMBOL" value={symbol} onChange={(e) => setSymbol(e.target.value.toUpperCase())} maxLength={10} disabled={tokenVerified}/><input className="forge-input sm:col-span-2" placeholder="Total supply" inputMode="numeric" value={supply} onChange={(e) => setSupply(e.target.value.replace(/[^0-9]/g, ""))} disabled={tokenVerified}/><input className="forge-input sm:col-span-2" placeholder="Metadata URI (IPFS/Arweave)" value={metadataUri} onChange={(e) => setMetadataUri(e.target.value)} disabled={tokenVerified}/><div className="sm:col-span-2 rounded-xl border border-white/10 bg-black/10 p-4 text-xs leading-5 text-white/45">Decimals are fixed at 9. The server converts whole-token supply to SPL base units when preparing the transaction.</div></div>}
      {step === 1 && <div className="mt-5 space-y-3">{[["Revoke mint authority", revokeMint, setRevokeMint], ["Revoke freeze authority", revokeFreeze, setRevokeFreeze], ["Make metadata immutable", immutable, setImmutable]].map(([label, checked, setChecked]) => <label key={label as string} className="flex cursor-pointer items-center justify-between rounded-xl border border-white/10 bg-black/10 p-4"><span><span className="block text-sm font-bold">{label as string}</span><span className="mt-1 block text-xs text-white/40">This becomes an irreversible on-chain state after signing.</span></span><input type="checkbox" checked={checked as boolean} disabled={tokenVerified} onChange={(e) => (setChecked as (value: boolean) => void)(e.target.checked)} className="h-5 w-5 accent-[#f5c542]"/></label>)}<div className="rounded-xl border border-[#f5c542]/15 bg-[#f5c542]/5 p-4 text-xs leading-5 text-white/55">FORGE X currently requires all three protections. The server enforces them independently of this UI.</div></div>}
      {step === 2 && <div className="mt-5 grid gap-3 sm:grid-cols-2"><label className="text-xs text-white/45">Initial SOL liquidity<input className="forge-input mt-2" inputMode="decimal" value={initialSol} onChange={(e) => setInitialSol(e.target.value)} disabled={poolVerified}/></label><label className="text-xs text-white/45">Initial token liquidity<input className="forge-input mt-2" inputMode="numeric" value={initialTokens} onChange={(e) => setInitialTokens(e.target.value.replace(/[^0-9]/g, ""))} disabled={poolVerified}/></label><label className="sm:col-span-2 flex items-center justify-between rounded-xl border border-white/10 p-4"><span><span className="block text-sm font-bold">Burn LP position</span><span className="mt-1 block text-xs text-white/40">Requested for the later verified LP-burn transaction.</span></span><input type="checkbox" checked={burnLp} onChange={(e) => setBurnLp(e.target.checked)} disabled={poolVerified} className="h-5 w-5 accent-[#f5c542]"/></label><div className="sm:col-span-2 rounded-xl border border-white/10 p-4 text-xs leading-5 text-white/45">The pool transaction is prepared and simulated server-side, then signed by your wallet. The pool is not considered created until its account is verified on-chain.</div></div>}
      {step === 3 && <div className="mt-5 space-y-3"><div className="rounded-xl border border-white/10 p-4"><p className="text-xs uppercase tracking-[.18em] text-white/35">Token</p><p className="mt-2 font-bold">{name || "Unnamed token"} · {symbol || "—"}</p><p className="mt-1 text-sm text-white/45">{supply || "0"} tokens · 9 decimals</p></div><div className="rounded-xl border border-white/10 p-4"><p className="text-xs uppercase tracking-[.18em] text-white/35">Safety</p><p className="mt-2 text-sm text-white/65">Mint revoked · Freeze revoked · Metadata immutable</p></div><div className="rounded-xl border border-white/10 p-4"><p className="text-xs uppercase tracking-[.18em] text-white/35">Liquidity</p><p className="mt-2 text-sm text-white/65">{initialSol} SOL + {initialTokens} tokens {burnLp ? "· LP burn requested" : "· LP remains active"}</p></div><div className="rounded-xl border border-[#f5c542]/15 bg-[#f5c542]/5 p-4 text-xs leading-5 text-white/55">{poolVerified ? "Token and Raydium CPMM pool are both verified on-chain." : tokenVerified ? "Token is verified. The next wallet signature creates the Raydium CPMM pool." : "Token creation and pool creation are separate signed transactions; neither is reported as verified until chain state confirms it."}</div></div>}
      ...
      <div className="mt-5 flex gap-3"><button type="button" onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0 || busy || tokenVerified} className="rounded-xl border border-white/10 px-5 py-3 text-sm font-bold disabled:opacity-30">Back</button>{step < STEPS.length - 1 ? <button type="button" onClick={() => setStep(step + 1)} disabled={!canContinue || busy || tokenVerified} className="flex-1 rounded-xl bg-[#f5c542] px-5 py-3 text-sm font-bold text-black disabled:opacity-30">Continue</button> : <button type="button" onClick={actionIsPool ? executePoolCreation : executeTokenCreation} disabled={busy || poolVerified} className="flex-1 rounded-xl bg-[#f5c542] px-5 py-3 text-sm font-bold text-black disabled:opacity-30">{status === "preparing" ? "Preparing…" : status === "signing" ? "Approve in wallet…" : status === "confirming" ? "Confirming on-chain…" : poolVerified ? "Launch verified ✓" : actionIsPool ? "Create & verify pool" : "Create & verify token"}</button>}</div>
    </div>
  );
}
