  ...
  return (
    <Card>
      ...
      {result && <div className="mt-5 space-y-3">
        {result.verified && result.pools.length > 0 && <div className="flex items-center justify-between rounded-xl border border-emerald-400/20 bg-emerald-400/5 px-4 py-3"><span className="text-sm text-white/60">Verification</span><span className="text-sm font-black text-emerald-300">✓ CPMM PROGRAM VERIFIED</span></div>}
        {result.pools.length === 0 ? <p className="text-sm text-white/45">No verified Raydium CPMM pool was found.</p> : result.pools.map((pool) => <PoolCard key={pool.id} pool={pool} network={result.network} />)}
      </div>}
    </Card>
  );
}
...
function PoolCard({ pool, network }: { pool: Pool; network: string }) {
  return <div className="rounded-xl border border-white/10 bg-white/[.02] p-4">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold">{pool.symbolA || "Token"} / {pool.symbolB || "SOL"}</p><p className="mt-1 break-all text-xs text-white/35">{pool.id}</p></div><a href={`https://solscan.io/account/${pool.id}${network === "devnet" ? "?cluster=devnet" : ""}`} target="_blank" rel="noreferrer" className="text-xs font-bold text-[#f5c542]">View on Solscan ↗</a></div>
    <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4"><Metric label="TVL" value={formatNumber(pool.tvl)} /><Metric label="24h volume" value={formatNumber(pool.volume24h)} /><Metric label="24h APR" value={formatPercent(pool.apr24h)} /><Metric label="Fee" value={formatPercent(pool.feeRate)} /></div>
  </div>;
}
