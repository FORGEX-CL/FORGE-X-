...
function PoolCard({ pool, network }: { pool: Pool; network: string }) {
  const tradeUrl = `/trade?pool=${encodeURIComponent(pool.id)}${pool.mintA ? `&inputMint=${encodeURIComponent(pool.mintA)}` : ""}`;
  return <div className="rounded-xl border border-white/10 bg-white/[.02] p-4"><div className="flex flex-wrap items-center justify-between gap-3"><div><p className="font-bold">{pool.symbolA || "Token"} / {pool.symbolB || "SOL"}</p><p className="mt-1 break-all text-xs text-white/35">{pool.id}</p></div><div className="flex items-center gap-3"><a href={tradeUrl} className="rounded-lg bg-[#f5c542] px-3 py-2 text-xs font-black text-black">Trade ↗</a><a href={`https://solscan.io/account/${pool.id}${network === "devnet" ? "?cluster=devnet" : ""}`} target="_blank" rel="noreferrer" className="text-xs font-bold text-[#f5c542]">View on Solscan ↗</a></div></div><div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4"><Metric label="TVL" value={formatNumber(pool.tvl)} /><Metric label="24h volume" value={formatNumber(pool.volume24h)} /><Metric label="24h APR" value={formatPercent(pool.apr24h)} /><Metric label="Fee" value={formatPercent(pool.feeRate)} /></div></div>;
}
